<template>
  ...
  <v-layout v-if="show">
    ...
    <v-app-bar color="side" elevation="3">
      ...
      <v-spacer></v-spacer>
      <v-form>
        <v-select :items="lan" v-model="languageChange" prepend-inner-icon="mdi-translate" class="mt-5 mr-5"
          variant='outlined'>
        </v-select>
      </v-form>
    ...
    <v-navigation-drawer color="side" v-model="sidebar">
      ...
      <v-list v-for="item in views[0]" :key="item.value" @click="mainContent(item)" color="rgb(3, 251, 106)"
        class="sidebarList" :prependIcon="item.prependIcon">
