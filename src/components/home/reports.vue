<template>
    <v-container class="main" :fluid=side>
        ...
        <v-data-table :items="leaveRequest">
...
export default {
    ...
    methods: {
        async decline(item) {
            ...
            const response = await this.axios.get('https://planner-node.onrender.com/leave');
            ...
            response.data.forEach((e) => {
                ...
                if (e.ids == item.id) {
                    this.axios.delete(`https://planner-node.onrender.com/leave/${e._id}`)
                }
            })
        ...
        async decline2(item) {
            await this.axios
                .post('https://planner-node.onrender.com/leaveapproval', {
